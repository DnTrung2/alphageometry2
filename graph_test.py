...
class GraphTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    txt = 'a b c = triangle a b c; h = orthocenter a b c; h1 = foot a b c; h2 = foot b c a; h3 = foot c a b; g1 g2 g3 g = centroid g1 g2 g3 g a b c; s = semicircle a b c ? coll h g s'    
  ...
  def test_build_graph_points(self):
    ...
    self.assertCountEqual(
        all_names,
        ['a', 'b', 'c', 'g', 'h' , 's', 'g1', 'g2', 'g3', 'h1', 'h2', 'h3'],  # Includes semicircle 's'
    )

  def test_build_graph_predicates(self):
    gr = GraphTest.g

    a, b, c, g, h, s, g1, g2, g3, h1, h2, h3 = gr.names2points(
        ['a', 'b', 'c', 'g', 'h' , 's', 'g1', 'g2', 'g3', 'h1', 'h2', 'h3']
    )

    # Explicit statements for both Circle and SemiCircle:
    ...
    self.assertTrue(gr.check_cong([s, a, s, b]))  # Semicircle congruence test
    ...
    # These are automatically inferred by the graph data structure:
  ...
  def test_enumerate_semicircles(self):
    g = GraphTest.g

    # Semicircle checks:
    for a, b, c in g.all_semicircles():
      self.assertTrue(g.check_semicircle([a, b, c]))  # Ensure points lie on semicircle boundary
      self.assertTrue(nm.check_semicircle([a.num, b.num, c.num]))
